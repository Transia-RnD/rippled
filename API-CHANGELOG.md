...
- `ledger_entry`: Added a `token_issuance` lookup (an object with `issuer` and `currency`, or a directly-specified index) returning the new `TokenIssuance` ledger entry introduced by the `TokenIssuance` amendment. `account_objects` recognizes `token_issuance` as a type filter and as a deletion blocker. The amendment also adds the `TokenIssuanceCreate`, `TokenIssuanceSet`, `TokenIssuanceDestroy`, and `TokenConvert` transactions and the `tecSUPPLY_EXCEEDED` result code.
