    ...
    beast::Journal const& j)
{
    ...
    if (created_ != 0 && !hasPrivilege(tx, CreateTokenIssuance))
    {
        JLOG(j.fatal()) << "Invariant failed: TokenIssuance created without privilege";
        return false;
    }

    if (deleted_ != 0 && !hasPrivilege(tx, DestroyTokenIssuance))
    {
    ...
    if (hasPrivilege(tx, CreateTokenIssuance) && (created_ != 1 || deleted_ != 0))
    {
    ...
    if (hasPrivilege(tx, DestroyTokenIssuance) && (deleted_ != 1 || created_ != 0))
    {
