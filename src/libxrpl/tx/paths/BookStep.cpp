...
private:
        ...
        , strandDeliver_(ctx.strandDeliver)
    {
        ...
        for (auto const ct : protocolCurveTypes)
        {
            ...
            ammLiquidities_.push_back(std::make_unique<AMMLiquidity<TIn, TOut>>(
                ctx.view,
                ammAcct,
...
public:
    ...
    qualityThreshold(Quality const& lobQuality) const
    {
        ...
        if (!this->ammLiquidities_.empty() &&
            !this->ammLiquidities_.front()->multiPath() &&
