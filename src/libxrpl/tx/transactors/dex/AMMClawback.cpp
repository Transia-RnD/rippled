...
AMMClawback::applyGuts(Sandbox& sb)
{
    ...
    if (curveType == CtBinned)
    {
        ...
        Number targetClaw =
            clawAmount ? std::min(Number{*clawAmount}, holderClaim) : holderClaim;
        ...
        for (auto const& s : slices)
        {
            ...
            Number const binClaim =
                Number{s.binReserveClaw} *
            ...
            std::uint64_t const sharesBurn = static_cast<std::uint64_t>(
                static_cast<std::int64_t>(
                    Number{static_cast<std::int64_t>(s.lpShares)} * frac));
            if (sharesBurn == 0)
                continue;

            // Bin reserves taken out: proportional on both sides so the
            // bin's price invariant is preserved.
            Number const burnFrac =
                Number{static_cast<std::int64_t>(sharesBurn)} /
                Number{static_cast<std::int64_t>(s.outstanding)};
            ...
            binSle->setFieldAmount(
                reserveClawField, s.binReserveClaw - drainClaw);
            binSle->setFieldAmount(
                reservePairField, s.binReservePair - drainPair);
            binSle->setFieldU64(
                sfOutstandingAmount, s.outstanding - sharesBurn);
            sb.update(binSle);
            ...
            (*issSle)[sfOutstandingAmount] =
                issOut >= sharesBurn ? issOut - sharesBurn : 0;
            sb.update(issSle);
        }
        return tesSUCCESS;
    }
