...
AMMDeposit::preflight(PreflightContext const& ctx)
{
    ...
    else
    {
        ...
        {
            JLOG(ctx.j.debug()) << "AMM Deposit: range/bin fields not allowed for non-CL/Binned pool.";
            return temMALFORMED;
        }
    }
...
AMMDeposit::applyGuts(Sandbox& sb)
{
    ...
    if (curveType == CtConcentratedLiquidity)
    {
        ...
        posSle->setFieldNumber(
            sfFeeGrowthInsideLast0, STNumber{sfFeeGrowthInsideLast0, Number{0}});
        posSle->setFieldNumber(
            sfFeeGrowthInsideLast1, STNumber{sfFeeGrowthInsideLast1, Number{0}});
        ...
        for (auto const tick : {*tickLower, *tickUpper})
        {
            ...
            if (newlyInitialised)
            {
                ...
                if (auto const ter =
                        setTickBitmap(sb, ammSle->key(), tick, ctx_.journal);
                    !isTesSuccess(ter))
                    return {ter, false};
            }
    ...
    if (curveType == CtBinned)
    {
        ...
        if (outstandingBefore == 0)
        {
            ...
            auto const activeBinSle =
                sb.read(keylet::ammBin(ammSle->key(), activeNow));
            bool const activeIsEmpty = !activeBinSle ||
                activeBinSle->getFieldU64(sfOutstandingAmount) == 0;
            if (activeIsEmpty)
