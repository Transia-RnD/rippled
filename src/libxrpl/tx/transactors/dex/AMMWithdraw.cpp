...
AMMWithdraw::preflight(PreflightContext const& ctx)
{
    ...
    bool const isBinnedPartial = (earlyCurveType == CtBinned) &&
        ctx.tx.isFieldPresent(sfShares) &&
    ...
    else
    {
        ...
        if (ctx.tx.isFieldPresent(sfPositionID) ||
            ctx.tx.isFieldPresent(sfPositionLiquidity) ||
...
AMMWithdraw::preclaim(PreclaimContext const& ctx)
{
    ...
    else if (curveType == CtBinned)
    {
        ...
        auto const mptokenSle =
            ctx.view.read(keylet::mptoken(mptId, accountID));
        if (!mptokenSle)
            return tecNO_ENTRY;
...
AMMWithdraw::applyGuts(Sandbox& sb)
{
    ...
    if (curveType == CtConcentratedLiquidity)
    {
        ...
        for (auto const tick : {tickLower, tickUpper})
        {
            ...
            if (gross == 0)
            {
                ...
                sb.erase(tickSle);
                if (auto const ter =
                        clearTickBitmap(sb, ammSle->key(), tick, ctx_.journal);
                    !isTesSuccess(ter))
                    return {ter, false};
            }
            else
        ...
        else
        {
            ...
            ammSle->setFieldU32(
                sfPositionCount, positions > 0 ? positions - 1 : 0);
        }
    ...
    if (curveType == CtBinned)
    {
        ...
        auto const isFullBurn =
            (ctx_.tx.getFlags() & tfWithdrawAll) == tfWithdrawAll;
        ...
        auto const holdingKeylet =
            keylet::ammBinHolding(ammSle->key(), accountID_, binID);
        ...
        if (holdingSle)
        {
            Number const fg0Last =
                Number{holdingSle->getFieldNumber(sfFeeGrowthInsideLast0)};
            Number const fg1Last =
                Number{holdingSle->getFieldNumber(sfFeeGrowthInsideLast1)};
            ...
            auto const feeAmt0 = owed0 > Number{0}
                ? toSTAmount(reserve0.asset(), owed0)
                : STAmount{reserve0.asset(), 0};
            auto const feeAmt1 = owed1 > Number{0}
                ? toSTAmount(reserve1.asset(), owed1)
            ...
            holdingSle->setFieldNumber(
                sfFeeGrowthInsideLast0,
                STNumber{sfFeeGrowthInsideLast0, fg0Now});
            holdingSle->setFieldNumber(
                sfFeeGrowthInsideLast1,
                STNumber{sfFeeGrowthInsideLast1, fg1Now});
        ...
        auto const frac =
            Number{static_cast<std::int64_t>(sharesToBurn)} /
