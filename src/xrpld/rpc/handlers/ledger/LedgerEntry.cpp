    ...
    [[maybe_unused]] unsigned const apiVersion)
{
    ...
    if (auto const value = LedgerEntryHelpers::hasRequired(params, {jss::issuer, jss::currency});
        !value)
    {
        return std::unexpected(value.error());
    }

    auto const issuer = LedgerEntryHelpers::parse<AccountID>(params[jss::issuer]);
    if (!issuer)
    {
        return LedgerEntryHelpers::invalidFieldError("malformedAddress", jss::issuer, "AccountID");
    }
