    ...
    [[maybe_unused]] unsigned const apiVersion)
{
    ...
    auto const issuer =
        ledger_entry_helpers::requiredAccountID(params, jss::issuer, "malformedIssuer");
    if (!issuer)
        return std::unexpected(issuer.error());
    ...
    [[maybe_unused]] unsigned const apiVersion)
{
    ...
    auto const scheduleID =
        ledger_entry_helpers::requiredUInt256(params, jss::coupon_schedule_id, "malformedRequest");
    if (!scheduleID)
    ...
    [[maybe_unused]] unsigned const apiVersion)
{
    ...
    auto const owner = ledger_entry_helpers::requiredAccountID(params, jss::owner, "malformedOwner");
    if (!owner)
        return std::unexpected(owner.error());
    auto const seq = ledger_entry_helpers::requiredUInt32(params, jss::seq, "malformedSeq");
    if (!seq)
    ...
    [[maybe_unused]] unsigned const apiVersion)
{
    ...
    if (!issuer)
    {
        return ledger_entry_helpers::invalidFieldError("malformedAddress", jss::issuer, "AccountID");
    }
    ...
    [[maybe_unused]] unsigned const apiVersion)
{
    ...
    auto const destination = ledger_entry_helpers::requiredAccountID(
