...
FirewallSet::preflight(PreflightContext const& ctx)
{
    ...
    {
        // create: check malformed transaction

        // sfCounterParty is required
        if (!ctx.tx.isFieldPresent(sfCounterParty))
        {
            JLOG(ctx.j.debug()) << "FirewallSet: sfCounterParty is required";
            return temMALFORMED;
        }

        // sfBackup is required
        if (!ctx.tx.isFieldPresent(sfBackup))
        {
            JLOG(ctx.j.debug()) << "FirewallSet: sfBackup is required";
            return temMALFORMED;
        }

        // sfNextOTPHash is required
        if (!ctx.tx.isFieldPresent(sfNextOTPHash))
        {
            JLOG(ctx.j.debug()) << "FirewallSet: sfNextOTPHash is required";
            return temMALFORMED;
        }
    }
    else
...
FirewallSet::doApply()
{
    ...
    if (!ctx_.tx.isFieldPresent(sfFirewallID))
    {
        ...
        sleFirewall->setFieldVL(sfNextOTPHash, ctx_.tx.getFieldVL(sfNextOTPHash));
        ...
        Keylet const preauthKeylet = keylet::withdrawPreauth(account_, backup);
