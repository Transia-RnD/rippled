...
struct PayChan_test : public beast::unit_test::suite
{
    ...
    testSimple(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(paychan::channelBalance(*env.current(), chan) == XRP(0));
        BEAST_EXPECT(paychan::channelAmount(*env.current(), chan) == XRP(1000));
        ...
        auto chanBal = paychan::channelBalance(*env.current(), chan);
        auto chanAmt = paychan::channelAmount(*env.current(), chan);
        BEAST_EXPECT(chanBal == XRP(0));
        BEAST_EXPECT(chanAmt == XRP(2000));

        {
            // bad amounts (non-xrp, negative amounts)
            // env(paychan::create(alice, bob, USDA(1000), settleDelay, pk),
            //     ter(temBAD_AMOUNT));
            // env(paychan::fund(alice, chan, USDA(1000)), ter(temBAD_AMOUNT));
            env(paychan::create(alice, bob, XRP(-1000), settleDelay, pk),
                ter(temBAD_AMOUNT));
        ...
        env(paychan::fund(
                alice,
                paychan::channel(alice, "noAccount", env.seq(alice) - 1),
                XRP(1000)),
            ter(tecNO_ENTRY));
        // not enough funds
        env(paychan::create(alice, bob, XRP(10000), settleDelay, pk), ter(tecUNFUNDED));
        ...
        //     env(paychan::claim(alice, chan, iou, iou), ter(temBAD_AMOUNT));
        //     env(paychan::claim(alice, chan, posXRP, iou), ter(temBAD_AMOUNT));
        //     env(paychan::claim(alice, chan, iou, posXRP), ter(temBAD_AMOUNT));
        //     env(paychan::claim(alice, chan, negXRP, negXRP), ter(temBAD_AMOUNT));
        //     env(paychan::claim(alice, chan, posXRP, negXRP), ter(temBAD_AMOUNT));
        //     env(paychan::claim(alice, chan, negXRP, posXRP), ter(temBAD_AMOUNT));
        // }
        {
            ...
            env(paychan::claim(alice, chan, reqBal, authAmt), ter(temBAD_AMOUNT));
        }
        {
            ...
            BEAST_EXPECT(paychan::channelBalance(*env.current(), chan) == reqBal);
            BEAST_EXPECT(paychan::channelAmount(*env.current(), chan) == chanAmt);
        ...
        }
        {
            ...
            env(paychan::claim(bob, chan, reqBal, authAmt, Slice(sig), alice.pk()));
            BEAST_EXPECT(paychan::channelBalance(*env.current(), chan) == reqBal);
            BEAST_EXPECT(paychan::channelAmount(*env.current(), chan) == chanAmt);
            auto const feeDrops = env.current()->fees().base;
            ...
            BEAST_EXPECT(paychan::channelBalance(*env.current(), chan) == chanBal);
            BEAST_EXPECT(paychan::channelAmount(*env.current(), chan) == chanAmt);
        ...
        }
        {
            ...
            auto const sig =
                signClaimAuth(alice.pk(), alice.sk(), chan, authAmt);
            env(paychan::claim(bob, chan, reqAmt, authAmt, Slice(sig), alice.pk()),
                ter(temBAD_AMOUNT));
            BEAST_EXPECT(paychan::channelBalance(*env.current(), chan) == chanBal);
            BEAST_EXPECT(paychan::channelAmount(*env.current(), chan) == chanAmt);
    ...
    testDisallowIncoming(FeatureBitset features)
    {
        ...
        {
            auto const chan = paychan::channel(bob, alice, env.seq(bob));
        ...
        {
            auto const chan = paychan::channel(cho, alice, env.seq(cho));
    ...
    testCancelAfter(FeatureBitset features)
    {
        ...
        auto const carol = Account("carol");
        {
            ...
            auto const chan = paychan::channel(alice, bob, env.seq(alice));
            env(paychan::create(alice, bob, channelFunds, settleDelay, pk, cancelAfter));
            BEAST_EXPECT(paychan::channelExists(*env.current(), chan));
            env.close(cancelAfter);
            {
                // dst cannot claim after cancelAfter
                auto const chanBal = paychan::channelBalance(*env.current(), chan);
                auto const chanAmt = paychan::channelAmount(*env.current(), chan);
        ...
        }
        {
            ...
            env(paychan::claim(carol, chan), txflags(tfClose), ter(tecNO_PERMISSION));
            BEAST_EXPECT(paychan::channelExists(*env.current(), chan));
            env.close(cancelAfter);
    ...
    testExpiration(FeatureBitset features)
    {
        ...
        env(paychan::fund(
            ...
            ter(temBAD_EXPIRATION));
        BEAST_EXPECT(!channelExpiration(*env.current(), chan));
        env(paychan::fund(alice, chan, XRP(1), NetClock::time_point{minExpiration}));
    ...
    testCloseDry(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(paychan::channelExists(*env.current(), chan));
        {
            ...
            auto const preBob = env.balance(bob);
            env(paychan::claim(alice, chan, channelFunds.value(), channelFunds.value()));
            BEAST_EXPECT(paychan::channelBalance(*env.current(), chan) == channelFunds);
    ...
    testDefaultAmount(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(paychan::channelExists(*env.current(), chan));
        {
            ...
            env(paychan::claim(bob, chan, reqBal, std::nullopt, Slice(sig), alice.pk()));
            BEAST_EXPECT(paychan::channelBalance(*env.current(), chan) == reqBal);
            auto const feeDrops = env.current()->fees().base;
            BEAST_EXPECT(env.balance(bob) == preBob + delta - feeDrops);
            chanBal = reqBal;
    ...
    testDepositAuth(FeatureBitset features)
    {
        ...
        auto USDA = alice["USD"];
        {
            ...
            auto const preBob = env.balance(bob);
            {
                ...
                env(paychan::claim(bob, chan, delta, delta), ter(temBAD_SIGNATURE));
                env.close();
            ...
            }
            {
                ...
                env(paychan::claim(alice, chan, delta, delta), ter(tecNO_PERMISSION));
                env.close();
    ...
    testDepositAuthCreds()
    {
...

        {
            ...
            env(paychan::claim(dillon, chan, delta, delta),
                credentials::ids({credIdx}),
                ter(tecBAD_CREDENTIALS));
...

        {
            ...
            env(paychan::claim(alice, chan, delta, delta), credentials::ids({credIdx}));
            env.close();
        }
    ...
    testMultiple(FeatureBitset features)
    {
        ...
        env(paychan::create(alice, bob, channelFunds, settleDelay, pk));
        BEAST_EXPECT(paychan::channelExists(*env.current(), chan1));
        auto const chan2 = paychan::channel(alice, bob, env.seq(alice));
        env(paychan::create(alice, bob, channelFunds, settleDelay, pk));
        BEAST_EXPECT(paychan::channelExists(*env.current(), chan2));
    ...
    testAccountChannelsRPC(FeatureBitset features)
    {
        ...
        auto const settleDelay = 3600s;
        auto const channelFunds = XRP(1000);
        auto const chan1Str = to_string(paychan::channel(alice, bob, env.seq(alice)));
        env(paychan::create(alice, bob, channelFunds, settleDelay, pk));
        env.close();
        {
        ...
        auto const chan2Str = to_string(paychan::channel(alice, bob, env.seq(alice)));
        env(paychan::create(alice, bob, channelFunds, settleDelay, pk));
        env.close();
        {
            auto const r =
    ...
    testAccountChannelsRPCMarkers(FeatureBitset features)
    {
...

        {
            ...
            {
                env(paychan::create(alice, b, channelFunds, settleDelay, alice.pk()));
            }
        }
    ...
    testAuthVerifyRPC(FeatureBitset features)
    {
        ...
        }
        {
            ...
            auto const chan =
                to_string(paychan::channel(charlie, alice, env.seq(charlie)));
            env(paychan::create(
    ...
    testOptionalFields(FeatureBitset features)
    {
        ...
        {
            auto const chan = to_string(paychan::channel(alice, bob, env.seq(alice)));
            env(paychan::create(alice, bob, channelFunds, settleDelay, pk));
            auto const r =
                env.rpc("account_channels", alice.human(), bob.human());
            BEAST_EXPECT(r[jss::result][jss::channels].size() == 1);
        ...
        }
        {
            ...
            auto const chan = to_string(paychan::channel(alice, carol, env.seq(alice)));
            env(paychan::create(
                alice,
                carol,
                channelFunds,
    ...
    testAccountDelete(FeatureBitset features)
    {
        ...
        for (bool const withOwnerDirFix : {false, true})
        {
            ...
            {
                auto const preAlice = env.balance(alice);
                env(paychan::claim(alice, chan, reqBal, authAmt), ter(tecNO_DST));
                env.close();
                BEAST_EXPECT(paychan::channelBalance(*env.current(), chan) == chanBal);
                BEAST_EXPECT(paychan::channelAmount(*env.current(), chan) == chanAmt);
                BEAST_EXPECT(env.balance(bob) == preBob);
            ...
            if (withOwnerDirFix)
            {
                ...
                BEAST_EXPECT(
                    paychan::channelAmount(*env.current(), chan) == chanAmt + XRP(1000));
    ...
    testUsingTickets(FeatureBitset features)
    {
        ...
        auto const pk = alice.pk();
        auto const settleDelay = 100s;
        auto const chan = paychan::channel(alice, bob, aliceTicketSeq);
        ...
        {
            auto const preAlice = env.balance(alice);
            env(paychan::fund(alice, chan, XRP(1000)), ticket::use(aliceTicketSeq++));
