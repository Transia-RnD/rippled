...
class NFToken_test : public beast::unit_test::suite
{
    ...
    testBrokeredAcceptDest(FeatureBitset features)
    {
        ...
        // test buyer is destination on sell offer
        {
            ...
            uint256 const buyOfferIndex =
                keylet::nftoffer(buyer, env.seq(buyer)).key;
            env(token::createOffer(buyer, nftID, XRP(315)), token::owner(minter));
            env.close();
            ...
            env(token::brokerOffers(broker, buyOfferIndex, sellOfferIndex), ter(tecNFTOKEN_BUY_SELL_MISMATCH));
            env.close();
