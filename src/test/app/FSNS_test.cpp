...
struct FSNS_test : public beast::unit_test::suite
{
    // void
    ...
    static Json::Value
    fsns_pin(jtx::Account const& account)
    {
        Json::Value jv;
        jv[jss::TransactionType] = jss::FSNSPin;
        jv[sfNickname.jsonName] = to_string(uint256{beast::zero});
...
BEAST_DEFINE_TESTSUITE(FSNS, app, ripple);
