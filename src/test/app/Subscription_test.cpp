...
struct Subscription_test : public beast::unit_test::Suite
{
    ...
    testSetPreclaimInvalid(FeatureBitset features)
    {
        ...
        */
        ...
        {
            env(subscription::create(alice, bob, XRP(10), 0s), Ter(temMALFORMED));
            env.close();
        }
        ...
        // temBAD_EXPIRATION: expiration in the past
        {
            ...
            env(subscription::create(alice, bob, XRP(10), 100s, expire), Ter(temBAD_EXPIRATION));
            env.close();
        }
        ...
        */
        ...
        // temBAD_EXPIRATION: update with past expiration
        {
            ...
            auto const expire = env.now() - 10s;
            env(subscription::update(alice, subId, XRP(100), expire), Ter(temBAD_EXPIRATION));
            env.close();
        }
    }
    ...
    testIOUPrecisionLoss(FeatureBitset features)
    {
        ...
        // Develop's canAdd tolerates sub-1e-4 relative loss, so this amount
        // that the original branch rejected with tecPRECISION_LOSS is now
        // accepted. The transactor's canAdd check is unchanged.
        {
            env(subscription::create(alice, bob, USD(1), 100s), Ter(tesSUCCESS));
            env.close();
    ...
    testMPTSetPreclaimInvalid(FeatureBitset features)
    {
        ...
        // tecOBJECT_NOT_FOUND: mpt does not exist
        {
            ...
            auto const mpt = test::jtx::MPT(alice.name(), makeMptID(env.seq(alice), alice));
    ...
    testMPTGateway(FeatureBitset features)
    {
        ...
        // Issuer as source
        {
            ...
            BEAST_EXPECT(env.balance(alice, MPT) == preAliceMPT + MPT(1000));
            // env.balance(issuer, MPT) returns outstanding negated on develop.
