...
struct Subscription_test : public beast::unit_test::suite
{
    ...
    testClaimPreclaimInvalid(FeatureBitset features)
    {
        ...
        // temMALFORMED: wrong currency/asset
        {
            auto const gw = Account{"gateway"};
            auto const USD = gw["USD"];
            ...
            env(subscription::claim(bob, subId, USD(1)), ter(temMALFORMED));
            env.close();
        ...
        // tecDST_TAG_NEEDED
        {
            ...
            // Clear destination tag in subscription then try to claim
            // This test is theoretical as we can't modify the subscription
            // to remove the tag after creation
        ...
        // tefFAILURE: subscription hasn't reached next payment time
        {
            auto const subId = getSubscriptionIndex(alice, bob, env.seq(alice));
            ...
            env(subscription::claim(bob, subId, XRP(10)), ter(tefFAILURE));
            env.close();
    ...
    void
    testMultiplePayments(FeatureBitset features)
    {
        testcase("multiple payments");
        using namespace jtx;
        using namespace std::literals::chrono_literals;

        // Multiple XRP payments
        {
            auto const alice = Account("alice");
            auto const bob = Account("bob");
            ...
            // Advance time and second payment
            env.close(100s);
            ...
            BEAST_EXPECT(
                env.balance(bob) ==
                preBob - (env.current()->fees().base * 2) + XRP(20));
    ...
    //         auto const subId = getSubscriptionIndex(alice, bob,
    //         env.seq(alice)); env(subscription::create(alice, bob, USD(100),
    //         100s)); env.close();
