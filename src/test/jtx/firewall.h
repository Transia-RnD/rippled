...
/** Sets the optional CounterParty on a JTx. */
class counter_party
{
private:
...
public:
    explicit counter_party(jtx::Account const& counterParty) : counterParty_(counterParty)
    {
    }
...
/** Sets the optional Backup on a JTx. */
class backup
{
private:
    jtx::Account backup_;
...
/** Sets the optional NextOTPHash on a JTx. */
struct next_otp
{
private:
    std::string value_;
...
public:
    explicit next_otp(Slice const& otp) : value_(strHex(otp))
    {
    }

    template <size_t N>
    explicit next_otp(std::array<std::uint8_t, N> const& c)
        : next_otp(makeSlice(c))
    {
    }
    ...
    {
        jt.jv[sfNextOTPHash.jsonName] = value_;
    }
};

/** Sets the optional CurrentOTP on a JTx. */
struct otp
{
private:
    std::string value_;
...
public:
    explicit otp(Slice const& otp) : value_(strHex(otp))
    {
    }

    template <size_t N>
    explicit otp(std::array<std::uint8_t, N> const& c)
        : otp(makeSlice(c))
    {
    }
    ...
    operator()(Env&, JTx& jt) const
    {
        jt.jv[sfCurrentOTP.jsonName] = value_;
    }
};
